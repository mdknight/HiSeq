...
if [ $# -ne 3 ]; then
    echo "Usage: human_rrna_SR.sh <read_1.fastq.gz> <number_of_reads> <output_file>"
    exit 1
fi
...
bwa mem -t 12 /opt/references/bwa_rRNA/human_rRNA.fa $TEMP/rrna_r1.fastq.gz | samtools view -@ 12 -bS -F 2304 -o $TEMP/rrna.bam -
...
echo -e "Read_1\tReads_checked\tHuman_rRNA_percent"
echo -e "${R1}\t${2}\t${RRNA_PERCENT}"

echo -e "Read_1,Reads_checked,Human_rRNA_percent" >> $OUTFILE
