...
if [ $# -ne 1 ]; then
    echo "Run in folder with .fastq.gz files. Usage: human_rrna.sh <project_id>"
    exit 1
fi
...
echo -e "Read_1,Reads_checked,Human_rRNA_percent" >> $REPDIR$OUTFILE
...
bwa mem -t 12 /opt/references/bwa_rRNA/human_rRNA.fa $TEMP/rrna_r1.fastq.gz \
